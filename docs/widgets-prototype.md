...
3. Click categories or brush histogram ranges. Kepler and the other widgets filter together. Native Kepler filters also affect widgets; use **Map layers** to edit layers and filters.
4. Use **Add chart** or panel settings to open SQLRooms' upstream forms inside the left panel, without a modal. Rename, configure, reorder, resize and remove panels. Removing all panels keeps the dashboard empty after reload. Suggestions are explicit after initialization.
...
The dashboard has no dataset selector. Choose a source in Add chart or widget settings. Changing a widget source moves only that widget; if the target lacks its field, select a new field in settings. Filters stay local to their source dataset.

## Panel behavior

Map layers and Widgets are mutually exclusive. The tabs form the pane header (supplied design variant 1a). Its right-hand arrow collapses only the body, keeping the selected tab and header position. Clicking either tab opens it; clicking the active tab keeps it open. Arrow keys move between tabs. The Widgets tab shows active widget-filter counts in a numeric badge even while Kepler is open. SQL tabs keep their original create/edit/execute and resize behavior. Add chart uses SQLRooms ChartBuilderContent inline; chart settings retain SQLRooms MosaicChartSettingsPanel. The title and dataset binding use a small report-level wrapper with readable dataset names.
