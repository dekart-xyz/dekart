...
Map settings and Widgets are mutually exclusive. Close widgets or click its active button to collapse it. The Widgets button shows active widget-filter counts even while Kepler is open. SQL tabs keep their original create/edit/execute and resize behavior. Add widget uses SQLRooms ChartBuilderContent inline; settings retain the upstream BlockSettingsPanel.
