...
import { AuthState, PlanType, RedirectState as DekartRedirectState } from 'dekart-proto/dekart_pb'
...
function AppRedirect ({ allowWorkspaceRedirect = true }) {
  ...
  const isReportView = /^\/reports\/[a-f0-9-]+\/?$/.test(location.pathname)
  const trialNotStarted = isCloud && userStream?.planType === PlanType.TYPE_PERSONAL
  ...
  if (
    ...
    userStream &&
    ((trialNotStarted && !isReportView && !['/workspace/trial', '/workspace/plan'].includes(location.pathname)) || (!userStream.planType && !isWorkspaceUrl && !isPlayground)) &&
...
function SwitchToPlayground () {
  ...
  const isCloud = useSelector(state => state.env.isCloud)
  useEffect(() => {
    // Cloud always operates in a real workspace; playground remains self-hosted only.
    dispatch(switchPlayground(!isCloud))
  }, [dispatch, isCloud])
  return null
}
...
export default function App () {
  ...
  const trialEndsAt = useSelector(state => state.workspace.subscription?.cancelAt)
  ...
  useEffect(() => {
    if (planType !== PlanType.TYPE_TRIAL || !trialEndsAt) return
    // cancelAt has second precision while the database deadline may include fractional seconds.
    const timeout = setTimeout(() => dispatch(getWorkspace()), Math.max(0, trialEndsAt * 1000 - Date.now() + 1000))
    return () => clearTimeout(timeout)
  }, [dispatch, planType, trialEndsAt])
