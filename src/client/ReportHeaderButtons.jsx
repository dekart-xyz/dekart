...
function EditModeButtons () {
  const dispatch = useDispatch()
  const { canWrite } = useSelector(state => state.report)
  const { saving, saveConflict } = useSelector(state => state.reportStatus)
        ...
        ? (
          <>
            ...
            <Button
              ...
              title={readOnly ? 'Workspace is read-only' : saveConflict || widgetsConflict ? 'Reload before saving' : saving ? 'Saving...' : 'Save this map'}
              type='text'
              ghost
              icon={saving || changed ? <CloudSyncOutlined /> : <CloudOutlined />}
              disabled={saving || readOnly || saveConflict || widgetsConflict}
              onClick={() => {
                track('SaveMap')
                dispatch(saveMap())
              }}
