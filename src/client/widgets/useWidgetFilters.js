...
export function useWidgetFilters (store, datasetId, ready, editing, pending) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const native = filters.filter(filter => !filter.id.startsWith('widget:')).map(filter => {
        ...
        return { filter, field, interactors, clients, record: getFilterRecord(datasetId, [filter], { cpuOnly: true, ignoreDomain: true }).cpu }
      })
      ...
      return store.getState().deferWidgetFilter(() => {
        try {
          ...
          for (const { filter, record, clients } of native) {
            ...
            const indices = copy.filterTable(record, layers, { cpuOnly: true, ignoreDomain: true }).filteredIndex
  ...
  useEffect(() => {
    ...
    for (const panel of panels || []) {
      ...
      // Preserve the canonical value while Kepler exposes a transient empty loading domain.
      if (filter && !filter.value?.length && previous?.value?.length) dispatch(setFilter(filterIndex, 'value', previous.value))
    }
  }, [ready, pending, panels, filters, dispatch])
