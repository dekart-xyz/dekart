  ...
  return (
    <>
      ...
      <div className={classnames(styles.reportCharts, { [styles.hidden]: builder || settingsOpen })}>
        {sources.filter(source => dashboards[source.id]).map(source => <DatasetCharts key={`${source.id}:${source.revision}`} store={store} source={source} showSource={sources.length > 1} onOpenData={onOpenData} editing={editing} />)}
...
function DatasetCharts ({ store, source, showSource, onOpenData, editing }) {
  const { error } = useWidgetFilters(store, source.id, Boolean(source.physical), editing)
