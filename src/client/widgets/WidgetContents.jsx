// REVIEW: Bind widget dashboards to live dataset schemas and immutable DuckDB revisions while presenting loading and compatibility states.
...
export default function WidgetContents ({ store, sources, loading, placeholderCount, onOpenData, editing }) {
  ...
  const missingDashboards = Object.values(dashboards).filter(dashboard => !sources.some(source => source.id === dashboard.id))
  return (
    <>
      ...
      <div className={classnames(styles.reportCharts, { [styles.hidden]: builder || settingsOpen })}>
        {sources.filter(source => dashboards[source.id]).map(source => <DatasetCharts key={source.id} store={store} source={source} showSource={sources.length > 1} onOpenData={onOpenData} editing={editing} />)}
        {missingDashboards.map(dashboard => <MissingDatasetCharts key={dashboard.id} store={store} dashboard={dashboard} sources={available} editing={editing} />)}
...
function MissingDatasetCharts ({ store, dashboard, sources, editing }) {
  const dispatch = useDispatch()
  const filters = useSelector(state => state.keplerGl.kepler?.visState.filters || [])
  const api = store.getState().mosaicDashboard
  const remove = panel => {
    api.removePanel(dashboard.id, panel.id)
    const filterIndex = filters.findIndex(filter => filter.id === `widget:${panel.id}`)
    if (filterIndex >= 0) dispatch(removeFilter(filterIndex))
    if (api.getDashboard(dashboard.id)?.panels.length === 0) api.removeDashboard(dashboard.id)
  }
  const rebind = (panel, datasetId) => {
    if (!datasetId) return
    api.ensureDashboard(datasetId, 'Widgets', 'grid')
    api.setSelectedTable(datasetId, `"memory"."widgets"."d_${datasetId.replaceAll('-', '_')}"`)
    api.addPanel(datasetId, panel)
    api.removePanel(dashboard.id, panel.id)
    if (api.getDashboard(dashboard.id)?.panels.length === 0) api.removeDashboard(dashboard.id)
    fitWidgetPanels(store, datasetId)
  }
  return (
    <section className={styles.datasetWidgets} data-testid='missing-widget-source'>
      <h3 className={styles.datasetLabel}>{dashboard.title || 'Unavailable dataset'}</h3>
      <div role='alert' className={styles.error}>The dataset for these charts is no longer available.</div>
      {dashboard.panels.map(panel => <div key={panel.id} className={styles.unavailablePanel}><span>{panel.title}</span>{editing && <><select aria-label={`Rebind ${panel.title}`} defaultValue='' onChange={event => rebind(panel, event.target.value)}><option value='' disabled>Choose dataset</option>{sources.map(source => <option key={source.id} value={source.id}>{source.label}</option>)}</select><Button variant='ghost' onClick={() => remove(panel)}>Delete</Button></>}</div>)}
    </section>
  )
}
...
function DatasetCharts ({ store, source, showSource, onOpenData, editing }) {
  const { error } = useWidgetFilters(store, source.id, Boolean(source.physical), editing, source.pending)
