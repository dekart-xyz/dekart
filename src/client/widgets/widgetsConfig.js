...
// Reading is conservative: unsupported persisted content remains an opaque string
// so an older client can keep saving the map without degrading chart definitions.
export function parseWidgetsConfig (raw) {
  if (!raw) return { config: null, raw: '', compatibility: 'supported' }
  try {
    ...
    return { config: root, raw, compatibility: 'supported' }
  } catch (error) {
    return { config: null, raw, compatibility: 'opaque', error: error.message }
  }
}
