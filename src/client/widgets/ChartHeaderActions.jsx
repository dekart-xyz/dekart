// REVIEW: Combine SQLRooms chart editing with reset behavior that clears matching Kepler filters in both directions.
...
export default function ChartHeaderActions (props) {
  ...
  return (
    <div className={styles.actions}>
      <ResetFiltersButton disabled={!hasActiveFilters && matchingFilters.length === 0} onClick={() => resetPanelFilters(reset, dispatch, matchingFilters)} aria-label={`Reset ${panel.title} filters`} tooltip='Reset panel filters' />
      {!readOnly && <DropdownMenu><DropdownMenuTrigger asChild><button aria-label='Chart actions' className={styles.trigger}><MoreVertical size={14} /></button></DropdownMenuTrigger><DropdownMenuContent align='end'><DropdownMenuItem onSelect={() => editPanel(selectBlock, openSettings, { dashboardId, panel, readOnly })}>Edit chart</DropdownMenuItem><DropdownMenuItem onSelect={() => { removePanel(dashboardId, panel.id); clearSelection(panel.id) }}>Delete chart</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}
    </div>
  )
}
