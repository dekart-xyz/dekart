...
export default function ReportWidgets ({ visible, editing, presentationPending, dataReloadPending, onOpenData }) {
  ...
  const adoptConfig = () => {
    ...
    restoreWidgetsConfig(store, widgets.config, datasetIds)
    const runtimeConfig = store.getState().mosaicDashboard.config
    for (const dashboardId of Object.keys(runtimeConfig.dashboardsById)) fitWidgetPanels(store, dashboardId)
    previousConfig.current = store.getState().mosaicDashboard.config
    adoptedConfig.current = widgets.config
    applying.current = false
    const pruned = serializeWidgetsConfig(previousConfig.current, widgets.config?.initialized)
    if (!editing || sameWidgetsConfig(pruned, widgets.config)) return
    adoptedConfig.current = pruned
    authoredConfigs.current.add(pruned)
  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      ...
      for (const dataset of datasetList) {
        ...
        if (generation !== prepareGeneration.current) return
        await connector.query(`CREATE OR REPLACE VIEW ${widgetTableName(dataset.id)} AS SELECT * FROM ${physical}`)
  ...
  useEffect(() => {
    ...
    if (initialized && editing && eligibleDatasetId) {
      if (!bindings.includes(eligibleDatasetId)) suggestWidgets(store, eligibleDatasetId, tables[eligibleDatasetId].fields)
      dispatch(widgetsDefaultsConsumed(eligibleDatasetId))
    }
  }, [initialized, editing, autoCreateWidgetIds, readySources, bindings, tables, store, dispatch])
    ...
    <RoomShell roomStore={store} className={styles.provider}>
      <RoomShell.DndProvider>
        <aside className={classnames(styles.panel, { [styles.hidden]: !visible, [styles.calculating]: calculating })} aria-label='Report charts' aria-busy={calculating}>
          ...
          {error ? <div role='alert' className={styles.error}>{error}</div> : <WidgetContents store={store} sources={sources} loading={!initialized || calculating} dataReloadPending={dataReloadPending} placeholderCount={Object.values(widgets.config?.config?.dashboardsById || {}).reduce((count, dashboard) => count + dashboard.panels.length, 0) || 3} onOpenData={onOpenData} editing={editing} />}
        </aside>
      </RoomShell.DndProvider>
    </RoomShell>
  )
