...
import { MosaicDashboard, ChartBuilderRoot, ChartBuilderContent, createMosaicDashboardChartPanelConfig, useMosaicClient, getMosaicDashboardSelectionName, Query } from '@sqlrooms/mosaic'
...
export default function ReportWidgets ({ visible, editing, onOpenData, onClose }) {
    ...
    <RoomShell roomStore={store} className={styles.provider}>
      <RoomShell.DndProvider>
        <aside className={classnames(styles.panel, { [styles.hidden]: !visible, [styles.viewer]: !editing })} aria-label='Report widgets'>
          <div className={styles.heading}><h2>Widgets</h2><button aria-label='Close widgets' onClick={onClose}><X size={16} /></button></div>
...
function DatasetWidgets ({ store, datasetId, editing, fields }) {
  ...
  return (
    <>
      ...
      {builder ? null : error || queryError ? <div role='alert' className={styles.error}>{error || queryError.message}</div> : editing && settingsOpen ? <div className={styles.settings} data-testid='widget-settings'><Button variant='ghost' onClick={() => store.getState().blockSettings.requestCloseSettingsPanel()}><ArrowLeft size={14} />Back to widgets</Button><BlockSettingsPanel onClose={() => store.getState().blockSettings.requestCloseSettingsPanel()} /></div> : dashboard ? <div className={styles.charts}><MosaicDashboard.Root dashboardId={datasetId} readOnly={!editing}><MosaicDashboard.Panels /></MosaicDashboard.Root></div> : <div className={styles.empty}><h3>No widgets yet</h3><p>Add a category or histogram to explore this dataset.</p></div>}
      {editing && !builder && <div className={styles.actions}><Button aria-label='Add widget' onClick={() => setBuilder(true)}><Plus size={15} />Add widget</Button><Button variant='ghost' onClick={() => suggestWidgets(store, datasetId, fields)}>Suggest widgets</Button></div>}
      <ChartBuilderRoot
        open={false} tableName={widgetTableName(datasetId)} columns={(table?.columns || []).filter(column => column.name !== '__dekart_row')} chartTypes={chartTypes} onCreateChart={(title, config) => {
      ...
      ><div className={classnames(styles.inlineBuilder, { [styles.hidden]: !builder })} data-testid='inline-widget-builder'><div className={styles.builderHeading}><h3>Add widget</h3><Button variant='ghost' onClick={() => setBuilder(false)}>Cancel</Button></div><ChartBuilderContent /></div>
      </ChartBuilderRoot>
    </>
  )
}
