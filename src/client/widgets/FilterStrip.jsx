...
export default function FilterStrip ({ visible, editing, disabled, onEdit, onApply }) {
  ...
  const chip = ({ filter, index }) => (
    <span className={styles.chip} key={filter.id}>
      <button className={styles.name} title={description(filter)} onClick={() => onEdit(index)} disabled={!editing || disabled}>{label(filter)}</button>
      <button className={styles.remove} aria-label={`Remove ${label(filter)} filter`} disabled={disabled} onClick={() => onApply(() => dispatch(removeFilter(index)))}><X size={11} /></button>
    </span>
  )
  return (
