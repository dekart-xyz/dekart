...
export default function FilterStrip ({ visible, editing, onEdit }) {
  ...
  const chip = ({ filter, index }) => (
    <span className={styles.chip} key={filter.id}>
      ...
      <button className={styles.remove} aria-label={`Remove ${label(filter)} filter`} onClick={() => dispatch(removeFilter(index))}><X size={11} /></button>
    </span>
  )
  return (
    <div ref={ref} className={styles.strip} style={{ display: visible ? undefined : 'none' }} data-testid='filter-strip' aria-label='Map and chart filters'>
      ...
      {entries.length > 0 && (
        <>
          ...
          <button className={styles.clear} onClick={() => filters.map((_, index) => index).reverse().forEach(index => dispatch(removeFilter(index)))}>Clear all</button>
        </>
      )}
