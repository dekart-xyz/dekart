...
function WelcomeScreen ({ setNextStep }) {
  ...
  return (
    <Result
      ...
      // icon={<RocketTwoTone />}
      title='Get started!'
      subTitle='Create workspace to connect and query private BigQuery datasets.'
      extra={(
        <>
          ...
          <div className={styles.playground}><Tooltip placement='bottom' title='Try Dekart with BigQuery public datasets'><Button type='link' onClick={() => dispatch(switchPlayground(true))}>Switch to Public Playground</Button></Tooltip></div>
        </>
             )}
    />
  )
