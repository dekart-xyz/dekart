  ...
  switch (action.type) {
    ...
    case 'reportUpdate': {
      ...
      if (state.revision > state.savedRevision) {
        ...
        if (sameConfig(action.report.widgetsConfig, state.raw) || sameConfig(action.report.widgetsConfig, state.savedRaw)) return { ...state, savedRaw: action.report.widgetsConfig, versionId: action.report.versionId, conflict: false }
