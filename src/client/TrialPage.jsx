...
const contactURL = 'https://calendly.com/vladi-dekart/30min'
...
export default function TrialPage () {
  ...
  const isEligible = isCloud && !isPlayground && !isDefaultWorkspace && userStream?.planType === PlanType.TYPE_PERSONAL

  useEffect(() => {
    // Track the acknowledgement only after the user stream establishes the viewer's role.
    if (!isEligible || tracked.current) return
    track(isAdmin ? 'TrialAckShown' : 'TrialAckBlockedNonAdmin')
  ...
  let title = 'Ask your workspace admin to start the trial'
  let subtitle = 'This workspace is on the free plan and is read-only until someone with admin access starts the 14-day trial.'
  if (isAdmin) {
    // Neutral copy keeps the page actionable until workspace details select the new or existing-workspace variant.
    title = workspace.hasReports === null ? 'Start your 14-day trial' : 'Your 14-day trial'
    subtitle = 'Unlimited maps, editors, viewers and database connections.'
    if (workspace.hasReports) {
      title = 'Dekart Cloud is now trial-based'
      subtitle = 'Your maps are safe. The free 3-map plan is replaced by a 14-day trial with no limits: unlimited maps, editors, viewers and database connections.'
    }
  }
          ...
          {isAdmin
            ? (
              <div className={styles.action}>
                ...
                <Button id='dekart-start-trial' type='primary' size='large' loading={loading} onClick={startTrial}>Start 14-day trial</Button>
