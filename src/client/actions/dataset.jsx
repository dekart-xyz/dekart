...
import { removeDatasetWidgets, widgetDatasetCreated } from './widgets'
...
export function createDataset (reportId) {
  return (dispatch) => {
    ...
    // REVIEW: Mark only datasets created by this UI request as eligible for automatic default widgets.
    dispatch(grpcCall(Dekart.CreateDataset, request, response => {
      dispatch(widgetDatasetCreated(response.id))
    }))
  }
}
...
export function removeDataset (datasetId, silent = false) {
  return async (dispatch, getState) => {
    ...
    dispatch(grpcCall(Dekart.RemoveDataset, request, (res) => {
      // REVIEW: Remove deleted dataset state from both Kepler and widgets before reporting successful completion.
