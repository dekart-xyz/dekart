...
export function reportUpdate (reportStreamResponse) {
  ...
  return async (dispatch, getState) => {
    ...
    // Normalize browser-owned state before comparing the incoming report revision
    // so deletion preserves pending edits on datasets that still exist.
    removedDatasetIds.forEach(datasetId => dispatch(cleanupRemovedDataset(datasetId, { removeWidgets: false })))
    dispatch({
    ...
    // Reconcile legacy widget bindings discovered during initial hydration.
    if (initialHydration) {
      const hydratedWidgets = getState().widgets
      Object.keys(hydratedWidgets.config.config.dashboardsById).forEach(datasetId => {
        if (!currentDatasetIds.has(datasetId)) dispatch(cleanupRemovedDataset(datasetId))
      })
    }
