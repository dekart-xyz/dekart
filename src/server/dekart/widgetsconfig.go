...
// Unreadable stored configuration is left untouched: dataset deletion must not
// depend on a dashboard this server version can parse.
func removeWidgetDataset(value, datasetID string) (string, bool, error) {
	...
	dashboards, ok := config["dashboardsById"].(map[string]any)
	if !ok {
		return value, false, nil
	}
	if _, exists := dashboards[datasetID]; !exists {
		return value, false, nil
	}
