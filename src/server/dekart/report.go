...
func (s Server) commitReportWithDatasets(
	...
	//Remap map and widget dataset bindings together before writing a forked report.
...
// validateExpectedReportVersion applies the report revision compare-and-swap:
// a client must send the revision it observed, and it must still be current.
func validateExpectedReportVersion(currentVersion string, expectedVersion *string) error {
	if expectedVersion == nil {
		return status.Error(codes.FailedPrecondition, "Reload this report before saving.")
	}
...
func (s Server) UpdateReport(ctx context.Context, req *proto.UpdateReportRequest) (*proto.UpdateReportResponse, error) {
	...
	if err := validateExpectedReportVersion(currentVersion.String, req.ExpectedVersionId); err != nil {
		return nil, err
	}
...
func (s Server) RestoreReportSnapshot(ctx context.Context, req *proto.RestoreReportSnapshotRequest) (*proto.RestoreReportSnapshotResponse, error) {
	...
	// Create the restoration snapshot inside the same transaction as the restored report content.
	...
	// Commit restored content and its snapshot atomically before notifying report subscribers.
	if err := tx.Commit(); err != nil {
