...
func (s Server) UpdateReport(ctx context.Context, req *proto.UpdateReportRequest) (*proto.UpdateReportResponse, error) {
	...
	if req.WidgetsConfig != nil {
		...
		widgetsConfig, err := reconcileWidgetDatasetsTx(ctx, tx, req.ReportId, req.GetWidgetsConfig())
		if err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
		if _, err := tx.ExecContext(ctx, "UPDATE reports SET widgets_config=$1 WHERE id=$2", widgetsConfig, req.ReportId); err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
	}
