...
func TestRemoveWidgetDatasetPreservesUnknownConfig(t *testing.T) {
	unknown := `{"version":2,"provider":"future","config":{}}`
	updated, changed, err := removeWidgetDataset(unknown, "dataset-old")
	require.NoError(t, err)
	require.False(t, changed)
	require.Equal(t, unknown, updated)
}
