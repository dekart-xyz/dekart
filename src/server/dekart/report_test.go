...
func TestValidateExpectedReportVersion(t *testing.T) {
	...
	stale := "version-stale"
	require.NoError(t, validateExpectedReportVersion(current, &current))
	require.Equal(t, codes.Aborted, status.Code(validateExpectedReportVersion(current, &stale)))
	require.Equal(t, codes.FailedPrecondition, status.Code(validateExpectedReportVersion(current, nil)))
}
