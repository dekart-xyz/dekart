...
describe('Widgets first production slice', () => {
  it('links Number and Category to one uploaded map and native filters', () => {
    ...
    cy.contains('button', 'Add chart').should('be.disabled')
    ...
    cy.get('.interval-x .overlay').last().scrollIntoView().then(overlay => {
      ...
      cy.get('.interval-x .overlay').last().trigger('mousedown', { clientX: bounds.left + scale.apply(25.5), clientY, button: 0, view })
      cy.get('body').trigger('mousemove', { clientX: bounds.left + scale.apply(34.5), clientY, buttons: 1, view })
      cy.get('body').trigger('mouseup', { clientX: bounds.left + scale.apply(34.5), clientY, button: 0, view })
    })
