...
describe('dataset source write timeout', () => {
  it('downloads a dataset that keeps streaming beyond the server timeout', () => {
    const query = "SELECT rows.i AS id, repeat('x', 8192) AS padding, 13.4 AS longitude, 52.5 AS latitude FROM generate_series(1, 20) AS rows(i) CROSS JOIN LATERAL (SELECT pg_sleep(0.3) WHERE rows.i IS NOT NULL) AS delay"
    ...
    cy.get('div:contains("20 rows")', { timeout: 120000 }).should('be.visible')
